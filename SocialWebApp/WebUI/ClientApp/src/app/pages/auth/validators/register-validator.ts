...
export function userNameValidator(field: AbstractControl): Validators | null {
  if (!field.value)
    return {other: `Please enter your user name`};
  if (!userNameChars.test(field.value))
    return {other: `The user name is invalid. Please use another one which not start with a number and not contain special characters, spaces or symbols`};
  if (field.value.length < 5)
    return {other: `The user name must be at least 5 characters long. Please use another one`};
  if (field.value.length > 32)
    return {other: `The user name is limited on the number of 32 characters. Please use another one`};
  return null;
}
...
export function passwordValidator(field: AbstractControl): Validators | null {
  ...
  if (field.value.length > 32)
    return {other: `The password is limited on the number of 32 characters. Please use another one`};
  return null;
}

export function emailValidator(field: AbstractControl): Validators | null {
  if (!field.value)
    return {other: `Please enter a valid email address`};
...
export function phoneValidator(field: AbstractControl): Validators | null {
  if(!field.value)
    return null;
  if(field.value.length < 11 || field.value.length > 15)
    return {other: `The phone number must be between 8 and 15 digits`};
  return null;
}
